...
#la préparation de notre fichier a trois paramètres, nb_slices,nmin,alpha
#Nous voulons trouver le nb_slice optimal, pour cela,
#utilisons la fonction compare_slicings

compare_slicings(df,ox,nmin=50,n1=1,n2=30) 
#compare_slicings affiche trois graphes:
#-un qui trace la densité volumique de points en fonction de nb_slices
#-un qui trace le nombre de données en fonction de nb_slices
#-un qui trace le nombre de cubes en fonction de nb_slices

#nous cherchons un maximum en nombre de cubes, de données et de densité
# Il en reste à l'utilisateur de trouver le nb_slice qui lui 
# convient le mieux en lisant le graphe 

#Par exemple, pour le module de Young, 
# on voit que nb_slice=12 n'est pas un mauvais choix
#car on a un maximum en nombre de cubes, et un nombre de 
# données raisonable 

#Ensuite je prépare mon fichier

preparator(file_name='ym',proprety='Young',
oxides=ox,nmin=50,alpha=0.1,nb_slices=12,
add_to_name='prepared')

#Si je ne suis pas content avec cette préparation
#je fais varier les autres paramètres, alpha, nmin.
#(par exemple, si je me retrouve avec pas assez de 
# données, je peux faire baisser nmin un peu)
