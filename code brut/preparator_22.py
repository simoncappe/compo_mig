...
def preparator(file_name,property,oxides,nmin=100,gap=0.6,alpha=0.1,add_to_name='_prepared',domains=False):
    ...
    data_list=nx.get_node_attributes(G,'data_node').values()
    prepared=pd.concat(data_list,ignore_index=True, sort=False)
...
preparator('density_table_tableonly_data_clean','Density',oxides,nmin=100,gap=0.6,alpha=0.1)
"""