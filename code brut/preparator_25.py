...
def data_sliceor(acc_data,acc_cubes,data,oxides,nmin,gap,limits):
    ...
    std_select=(len(limits)/100)*grby[oxides].std(ddof=0).mean(axis=1)>gap    
    ...
    if np.any(select):
        for next_one in cubes[select]:
            data_sliceor(acc_data,acc_cubes,grby.get_group(next_one)[initial_cols],\
                oxides,nmin,gap,np.array([100*i/(len(limits)+1) for i in range(1,len(limits)+2)]))
...
def preparator(file_name,property,oxides,nmin=100,gap=0.6,alpha=0.1,add_to_name='_prepared',domains=False,nb_slices_init=1):
    df=pd.read_csv(file_name+'.csv')
    acc_data={} ;  acc_cubes=[]
    limits=np.array([100*(i+1)/(nb_slices_init) for i in range(nb_slices_init)])
    #lance decoupage
    ...
    if acc_cubes==[]:
        ...
        if domains:
            ...
            f=open(file_name+add_to_name+'domains.txt','w')
...
#preparator('density_table_tableonly_data_clean','Density',oxides,nmin=50,gap=0.1,alpha=0.6,domains=True,nb_slices_init=5)